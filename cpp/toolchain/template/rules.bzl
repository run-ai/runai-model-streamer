...
load(":utils.bzl", "get_target_triplet", "runai_crosstool_tools")
...
def _get_include_directories(target_triplet, gcc_version, use_cross, arch):
    return (_cross_include_directories(target_triplet, gcc_version) if use_cross else _builtin_include_directories(target_triplet, gcc_version)) + [
        ...
        "/usr/include",
        # Add external dependencies as toolchain dependencies, to workaround absolute path inclusion
        # TODO: Remove this hack once https://github.com/bazelbuild/bazel/issues/24930 is resolved
        "/opt/%s-aws/include" % arch,
    ]
...
def _impl(ctx):
    ...
    return cc_common.create_cc_toolchain_config_info(
        ...
        # Add external dependencies as toolchain dependencies, to workaround absolute path inclusion
        # TODO: Remove this hack once https://github.com/bazelbuild/bazel/issues/24930 is resolved
        cxx_builtin_include_directories = _get_include_directories(target_triplet, ctx.attr.gcc_version, ctx.attr.use_cross, ctx.attr.arch)
    )
