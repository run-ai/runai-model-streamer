...
common::ResponseCode GCSClient::async_read(const char* path, common::backend_api::ObjectRange_t range, char* destination_buffer, common::backend_api::ObjectRequestId_t request_id)
{
    ...
    google::cloud::storage_experimental::BucketName bucket_name(uri.bucket);
    ...
    for (unsigned i = 0; i < size && !_stop; ++i)
    {
        ...
        _client.ReadObjectRange(
            ...
            if (result.ok())
            {
                ...
                // A failure to read any sub range fails the entire read request
                // If a previous request failed, skip parsing this response
                bool keep_processing = is_success->load();
                if (keep_processing) {
                    LOG(SPAM) << "Async read request " << request_id << " succeeded - " << running << " running";
                } else {
                    LOG(SPAM) << "Async read request " << request_id << " succeeded, but remaining responses will be skipped - " << running << " running";
                    return;
                }
                ...
                for (auto chunk : payload.contents()) {
                    ...
                    if (bytes_received > bytesize_) {
                        ...
                        LOG(ERROR) << "GCS read received at least " << bytes_received << " bytes, but only "
