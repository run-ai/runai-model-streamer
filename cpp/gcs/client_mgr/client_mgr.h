...
// Singleton clients manager for reusing clients of the current GCS bucket
...
T* ClientMgr<T>::pop(const common::s3::Path & path, const common::s3::Credentials_C & credentials)
{
...

    {
        ...
        while (!unused.empty())
        {
            ...
            // TODO: Check if credentials have not changed
            LOG(DEBUG) << "Reusing GCS client";
            return ptr;
        }
    }
    ...
    auto client = std::make_unique<T>(path);
...
{
    LOG(DEBUG) << "Releasing GCS client";
    auto & mgr = get();
    const auto guard = std::unique_lock<std::mutex>(mgr._mutex);
...
{
    LOG(DEBUG) << "Releasing all GCS clients";
    auto & mgr = get();
    ...
    {
        LOG(ERROR) << "There are used GCS clients - number of clients is " << mgr._clients.size() << " while number of unused clients is " << count;
        return;
    }
...
{
    LOG(DEBUG) << "Stopping all GCS clients";
    auto & mgr = get();
...
using GCSClientMgr = ClientMgr<GCSClient>;
